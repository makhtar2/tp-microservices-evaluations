...
exports.getQuestions = async (req, res) => {
  try {
    const { subject, chapter, difficulty, type, search, includeInactive, page = 1, limit = 20 } = req.query;
...
// Retrait de la banque active uniquement (règle métier : une question déjà
// utilisée dans une évaluation passée doit rester consultable, pas de
// suppression en cascade).
exports.deleteQuestion = async (req, res) => {
  try {
    const question = await Question.findByIdAndUpdate(req.params.id, { active: false });
    if (!question) return res.status(404).json({ error: 'Question introuvable' });
    res.status(204).send();
  } catch (err) {
    res.status(400).json({ error: 'Identifiant invalide' });
